...
Поиск книги по названию, автору или ISBN. Используется для автозаполнения формы добавления книги.

Источники (с приоритетом):
1. **Google Books API** — если задан `GOOGLE_BOOKS_API_KEY` в `.env` (бесплатно, 1000 req/day)
2. **Open Library** — fallback при отсутствии ключа или недоступности Google Books
...
Возвращает до 7 результатов. При недоступности обоих источников — пустой массив.
