...
export function BookSpine({ placement, bookcaseId }: BookSpineProps) {
  ...
  return (
    ...
    >
      ...
      {/* Кнопка снятия с полки.
          onPointerDown — останавливаем всплытие чтобы не запустить drag при клике. */}
      {isHovered && !isDragging && (
        <button
          ...
          disabled={isRemoving}
          title="Снять с полки"
          className="absolute top-1 left-1/2 -translate-x-1/2 z-10 rounded-full bg-black/50 hover:bg-red-600 disabled:opacity-50 transition-colors flex items-center justify-center"
